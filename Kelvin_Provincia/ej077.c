...
int main(void) {
    ...
    int moda[MAX_PERSONAS], m, frecuencia, frecuencia_moda, acumulador;
    ...
    if (personas > 0) {
        ...
        printf ( "\nMediana     : %.2f\n", mediana );
        j = 0;
        for (i = 0; i < MAX_PERSONAS; i += INTERVALO) {     // TABLA
            acumulador = 0;
            while(edad[j] < i + INTERVALO && j < personas ) {
                acumulador++;
                j++;
            }
            printf ( "%3d - %3d: %3d\n", i, i + INTERVALO - 1, acumulador );
        }
    } else {
